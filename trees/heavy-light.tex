...
HLD costă un factor suplimentar de $\bigoh(\log n)$, așadar genul de întrebări la care răspundem în $\bigoh(\log n)$ pe vector ne vor costa $\bigoh(\log^2 n)$ pe arbore.
...
\begin{itemize}
  ...
  \item LCA + \textit{binary lifting} rezolvă doar problema fără actualizări. Fiecare pointer notează și maximul nodurilor peste care trece. Dar la actualizare, dacă avem un nod cu $\bigoh(n)$ fii, vom fi nevoiți să actualizăm informația din $\bigoh(n)$ pointeri.

  \item Similar și pentru orice tentativă de descompunere în radical.

  \item Algoritmul lui Mo pe arbore funcționează, dar lent. Structura pentru intervalul curent trebuie să admită inserarea și ștergerea de valori și interogarea de maxim. Complexitatea va fi $\bigoh(q \sqrt{n} \log n)$.
\end{itemize}
