...
\subsection{Problema Cucuruz (Concurs RoAlgo \# 7)}
\label{problem:cucuruz}

\href{https://kilonova.ro/problems/1908}{enunț}
$\bullet$
\hyperref[code:cucuruz]{sursă}

O primă dificultate este să ne decidem în ce mod să agregăm datele pentru a le calcula.

Fie $C(u,v)$ suma valorilor de pe calea $u-v$. Facem întîi observația că fiecare pereche de noduri $(u, v)$ aflate la distanță cel mult $p$ trebuie luată în calcul la răspuns. Într-adevăr,

\begin{itemize}
  \item fie $u < v$ și atunci $C(u,v)$ contribuie la $Fss(v)$,
  \item fie $u > v$ și atunci $C(u,v)$ contribuie la $Fss(u)$.
\end{itemize}

S-ar putea să existe soluții bazate pe tehnica \textit{small-to-large} care pornesc de la această observație. Dar iată soluția bazată pe descompunere în centroizi. Ea amintește (pînă la un punct) de problema \hyperref[problem:fixed-length-paths-1]{Fixed-Length Paths I}.

\subsubsection*{Procesarea din fiecare centroid}

Cînd procesăm un centroid $u$ dorim să contabilizăm contribuția tuturor căilor de lungime $l \leq p$ care trec prin acel centroid (dar nu și prin alt centroid de nivel mai înalt). Restul căilor sînt complet cuprinse în subarborele unuia dintre fii și vor fi găsite la un moment viitor.

Ca să contabilizăm toate căile, vom considera că am procesat primii fii ai lui $u$ și că avem informațiile despre ei într-o structură (vedem imediat ce informații și ce structură). Acum procesăm și fiul următor, să-i spunem $v$. Dorim să modelăm faptul că orice cale din fiii anteriori de lungime $d \leq p$ poate fi combinată cu orice căi din subarborele lui $v$ de lungime cel mult $p - d$. Ca să fim eficienți, trebuie să încercăm să ținem totaluri pe nivel. Așadar, pentru fiecare adîncime $d$ între $0$ și $p$ vom ține $left[d]$ = suma costurilor $C(u, x)$ pentru toate căile $(u, x)$, din fiii procesați anterior, terminate în noduri $x$ care se află la adîncimea $d$ (raportat la nodul $u$). Similar vom ține $right[d]$ pentru căile de lungime $d$ din fiul curent.

Să studiem regula de combinare. Dacă am avea explicit patru căi în stînga cu sumele $a, b, c, d$ și trei căi în dreapta cu sumele $e, f, g$, atunci am obține prin combinare 12 căi cu sumele $a+e, a+f, a+g, b+e, \dots, d + g$. Observăm că fiecare dintre sumele $a, b, c, d$ trebuie adunată de 3 ori și fiecare dintre sumele $e, f, g$ trebuie adunată de 4 ori. De aici rezultă că:

\begin{enumerate}
  \item Nu este nevoie să ținem sume individuale, ci doar sumele în stînga $(a + b + c + d)$ și în dreapta $(e + f + g)$.

  \item Avem nevoie să ținem minte nu doar \textbf{sumele} costurilor căilor pe fiecare nivel, ci și \textbf{numărul} de căi pe fiecare nivel, fie ele $cnt\_left[d]$ și $cnt\_right[d]$.
\end{enumerate}

După ce calculăm contribuția la răspuns pentru aceste combinații între fiul $v$ și fiii anteriori, vom încorpora informația din $right$ în $left$. În principiu, facem o simplă adunare pe fiecare nivel, cu completarea că la $right[d]$ trebuie să mai adăugăm și $cnt\_right[d] \cdot V[u]$, deoarece la toate acele căi trebuie să adăugăm și nodul $u$ cu valoarea sa.

\subsubsection*{\textit{Back to basics}}

Cum spuneam, căile de adîncime $d \leq p$ din fiii anteriori trebuie combinate cu căile din subarborele fiului curent de lungime cel mult $p - d$, adică de adîncimi $0, 1, \dots, p - d$. Este tentant să ne gîndim la sume pe interval, iar de aici la AIB-uri.

Dar nu uitați de uneltele elementare! În primul rînd, vom parcurge complet fiul curent. Abia apoi vom face interogări de sumă pe prefix. Rezultă că ne sînt suficiente sumele parțiale peste vectorul $right$! Astfel eliminăm un factor de $\bigoh(\log n)$ și obținem o soluție în $\bigoh(n \log n)$.


\subsection{Problema LaNaic (Lot 2024)}
\label{problem:la-naic}

\href{https://kilonova.ro/problems/2830}{enunț}
$\bullet$
\hyperref[code:la-naic]{sursă}

\subsection*{Soluție în \texorpdfstring{$\bigoh(n^2)$}{O(n2)}}

Menționăm o soluție ineficientă, dar ușor de codat și care face o observație teoretică importantă. Lanțul optim trebuie să aibă un capăt undeva. Deci pornim cîte un DFS din fiecare nod ca să aflăm cît de departe putem ajunge pornind din acel nod și făcînd cel mult $m$ modificări. Pentru nodul curent, reținem:

\begin{itemize}
  \item Numărul de modificări necesare pentru a putea ajunge la el. Dacă depășim $m$ modificări, oprim DFS-ul.
  \item Lungimea sufixului de muchii de aceeași culoare din stiva DFS curentă. Oricînd această lungime depășește $k$, incrementăm numărul de modificări necesare și resetăm lungimea.
\end{itemize}

Observăm că numărul de modificări necesare pentru a sparge un lanț monocrom de lungime $l$ este întotdeauna $\lfloor \frac{l}{k + 1} \rfloor$.

\subsection*{Soluție în \texorpdfstring{$\bigoh(n \log n)$}{O(n log n)}}

Să considerăm o descompunere în centroizi a arborelui. Pentru centroidul curent $x$, ne punem întrebarea: care este lanțul $k$-alternant maxim care trece prin $x$? După cum știm, dacă rezolvăm acest pas în $\bigoh(S(x))$, unde $S(x)$ este mărimea subarborelui lui $x$, atunci complexitatea globală va fi $\bigoh(n \log n)$.

Construim o pereche de vectori $C_0$ și $C_1$, unde $C_i(j)$ este \textbf{costul minim} necesar pentru a construi un lanț $k$-alternant care pornește din $x$ pe o muchie de valoare $i$, are lungime (adîncime) $j$ și se termină undeva în subarborele lui $x$. Indicii $j$ iau valori între 0 și $H(x)$, unde $H(x)$ este înălțimea în muchii a subarborelui lui $x$.

Definim \textbf{costul} al unui astfel de lanț ca fiind o pereche: numărul minim de modificări necesare, iar, în caz de egalitate, lungimea minimă a prefixului de muchii de valoare $i$ cu care începe lanțul. Raționamentul din spatele acestei definiții este că lanțul optim prin $x$ va consta din două lanțuri (sau, în cazul degenerat, un singur lanț) care pornesc spre doi fii diferiți. Dacă cele două lanțuri pornesc pe prefixe de aceeași culoare, și dacă suma lungimilor acestor prefixe depășește $k$, este necesară o modificare suplimentară, altfel nu. Nu avem niciun motiv să preferăm un lanț care face mai multe modificări, dar care are prefix mai scurt. Putem amîna acele modificări pînă cînd și dacă devin necesare. Mai remarcăm și că vectorii $C$ sînt monotoni: costul crește cu adîncimea.

Pentru fiecare fiu $y$ putem calcula un astfel de vector, $C_y$. Există unul singur, deoarece muchia $(x,y)$ are fie valoarea 0, fie 1. Este necesară o parcurgere DFS. O implementare rezonabilă este: lansăm o primă parcurgere pe muchii de aceeași valoare cu muchia $(x,y)$, al cărei scop este să determine lungimea prefixului de aceeași valoare. Din ea comutăm la o a doua parcurgere DFS ori de cîte ori întîlnim prima muchie de valoare opusă.

Pe măsură ce fiii lui $x$ își încheie parcurgerile, $x$ are de făcut două operații:

\begin{enumerate}
  \item Să caute lanțul maxim între fiul curent $y$ și oricare din fiii anteriori.
  \item Să adauge informația din $C_y$ la $C_0$ sau $C_1$.
\end{enumerate}

Deoarece vectorii sînt monotoni, putem face pasul (1) în $\bigoh(H_{max}+ H(y))$, unde $H_{max}$ este adîncimea maximă a oricărui fiu anterior. Folosim tehnica \textit{two pointers} pentru a căuta suma maximă, dar nu mai mult de $M$, a două elemente din doi vectori sortați. Pasul (2) este trivial, constînd doar din compararea celor doi vectori poziție cu poziție.

Pentru complexitate optimă, este important să procesăm fiii lui $x$ în ordinea crescătoare a adîncimii. Altfel $H_{max}$ poate crește rapid, de exemplu dacă explorăm fiul cel mai adînc primul. După sortarea fiilor, efortul total pentru $x$ va fi $\bigoh(S(x))$. Sursa anexată sortează fiii naiv, cu \ccode{std::sort}. Am încercat și să fac sortarea \href{https://kilonova.ro/submissions/1162662}{în timp liniar}, distribuind fiii după adîncimi, dar diferența de timp este minoră.
