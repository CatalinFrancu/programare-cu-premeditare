...
Putem extinde arborii indexați binar la matrice, cu costuri $\bigoh(\log^2 n)$ pentru operații. Să considerăm următoarele operații (\textit{point update, rectangle query}):
...
\subsection{Construcția în \texorpdfstring{$\bigoh(n^2)$}{O(n²)}}

Și acest arbore poate fi construit in-place, refolosind matricea dată la intrare și în timp $\bigoh(1)$ per element.
