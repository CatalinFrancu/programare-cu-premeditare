...
Mai concis, putem folosi funcția \ccode{__builtin_clz(x)}, care ne spune cu cîte zerouri începe numărul \ccode{x}:
...
Apropo de \textit{clean code}: Remarcați că am denumit funcțiile \ccode{set} și \ccode{add}, nu le-am denumit pe ambele \ccode{update}. Astfel am evidențiat diferența dintre ele.
...
Există și o implementare recursivă, pe care nu o vom discuta acum (o menționez doar ca să o fac de rîs). O vom discuta în \hyperref[chapter:segment-trees-lazy]{capitolul următor}. Este păcat că mulți elevi învață și stăpînesc doar acea implementare, pe care o aplică și cînd nu este nevoie de ea, deși implementarea iterativă de mai sus este de 2-3 ori mai rapidă. Implementarea iterativă ar trebui să fie implementarea voastră de referință oricînd este suficientă.
