...
$$\frac{1}{g_i'! g_{i+1}'! \dots g_p'!}$$

unde exact $j$ dintre factorii $g'$ sînt \textbf{diferiți} de factorul $g$ corespunzător (adică am ales un număr prim din acea frecvență). Atunci pe poziția $i$ putem să alegem valoarea $g_i' = g_i$ sau valoarea $g_i' = g_i - 1$. Rezultă recurența:

$$d(i, j) = \frac{1}{g_i!} \cdot d(i + 1, j) + \frac{1}{(g_i - 1)!} \cdot d(i + 1, j - 1)$$

Aici, primul termen semnifică că \textbf{nu} am folosit un număr prim din frecvența $g_i$, deci de la $i+1$ încolo încă avem de ales $j$ numere prime, iar al doilea termen semnifică că am folosit un număr prim din frecvența $g_i$, deci de la $i+1$ încolo mai avem de ales $j-1$ numere prime. Răspunsul la problemă va fi $T \cdot d(1, n)$.
...
De aceea, este suficient să menținem produsul valorilor $p$ pentru pozițiile $p$ care conțin caracterul \texttt{?}. Aritmetica ține cu noi dacă indexăm șirul natural, de la 0. Apoi, ori de cîte ori poziția $pos$ capătă valoarea \texttt{?}, înmulțim acest produs cu $pos$. Ori de cîte ori poziția $pos$ capătă valoarea \texttt{<} sau \texttt{>} înmulțim produsul cu inversul lui $pos$.
...
$\bullet$
\hyperref[code:devu-and-flowers]{sursă}
...
Este tentant să încercăm următoarea abordare: pentru fiecare subsecvență $W = [l, r]$ adăugăm la răspuns cantitatea $C_{x+y}^x$, unde $x$ este numărul de zerouri, iar $y$ numărul de unu din $W$. Dar este incorect, căci vom număra și unele permutări care lasă nemodificate porțiuni din $W$. Din această cauză, două subsecvențe parțial suprapuse, $W$ și $W'$, vor număra de două ori permutările care modifică doar zona comună $W \cap W'$.

Să contabilizăm subsecvențele într-un mod care elimină duplicatele. Fie $[l,r]$ o subsecvență amestecată astfel încît $l$ și $r$ sînt prima, respectiv ultima poziție modificată. Secvența merită luată în calcul dacă întrunește patru condiții:

\begin{enumerate}
  \item Are lungime cel puțin 2. O secvență de lungime 1 nu poate fi amestecată. \emoji{thinking-face}

  \item Conține cel mult $k$ cifre 1. Mai multe nu avem voie să selectăm, dar putem selecta mai puține (ne prefacem că amestecăm o secvență care include $[l,r]$, dar lasă nemodificate prefixul și sufixul). Însă de aici decurge...

  \item Șirul original $s$ conține cel puțin $k$ cifre 1.

  \item Secvența conține cifrele necesare pentru a nega pozițiile $l$ și $r$. De exemplu, dacă $s[l] = s[r] = 1$, secvența trebuie să conțină și două zerouri.
\end{enumerate}
...
Putem optimiza soluția de mai sus astfel. Să fixăm doar capătul stîng, $l$. Atunci capătul drept $r$ poate fi oriunde începînd de la $l$ pînă la punctul în care include cel mult $k$ de 1 (plus toate zerourile următoare). Apoi, impunem doar două condiții:

\begin{enumerate}
  \item Șirul original $s$ conține cel puțin $k$ cifre 1 (ca mai sus).

  \item Undeva în $[l,r]$ există un caracter diferit de $s[l]$. Altfel nu ne putem  asigura că prima modificare este pe poziția $l$.
\end{enumerate}

Odată fixat capătul stîng, orice amestecare distinctă a caracterelor din $[l+1,r]$ este o soluție distinctă (numărată o singură dată). Dacă am grupa acum aceste amestecări după poziția ultimului caracter modificat, am regăsi subsecvențele $[l,x]$ din soluția anterioară.

Ca la orice problemă de numărare, merită să ne gîndim la întrebările:

\begin{enumerate}
  \item Sîntem siguri că am numărat toate posibilitățile?

  \item Sîntem siguri că nu am numărat unele posibilități de mai multe ori?
\end{enumerate}
