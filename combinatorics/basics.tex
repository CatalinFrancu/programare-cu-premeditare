...
Pentru valori mari, de regulă problema ne va cere să operăm modulo un număr prim. În această situație, putem precalcula inversele modulare ale tuturor factorialelor: \ccode{inv_fact[k]} = $(k!)^{-1} \bmod M$. Apoi, putem calcula în $\mathcal{O}(1)$ orice $C_n^k$.
...
Valoarea teoretică converge către $\frac{n!}{e}$.
...
\begin{itemize}
  \item Fie $n$ numărul de transformări posibile.
  \item Fie $C_r$ numărul de obiecte care rămîn nemodificate în urma unei transformări de tipul $r$.
  \item Atunci numărul de obiecte-unicat din colecție este $\frac{1}{n} \sum_r C_r$.
\end{itemize}

Pentru problema cu colierele de lungime $n$ și $k$ culori, definim transformarea $r$ drept rotirea colierului cu $r$ poziții. Atunci

\begin{itemize}
  \item $C_0 = k^n$, deoarece dacă nu rotim nimic există $k^n$ coliere distincte.
  \item $C_1 = k$, deoarece dacă un colier nu se modifică prin rotirea cu o poziție, atunci mărgeaua 1 are aceeași culoare cu mărgeaua 2, mărgeaua 2 are aceeași culoare cu mărgeaua 3 etc. Deci toate mărgelele au aceeași culoare.
  \item $C_2 = k$ dacă $n$ este impar. De exemplu, pentru $n = 7$, mărgeaua 1 va urma pozițiile 3, 5, 7, 2, 4, 6, 1. Din nou, este necesar ca toate mărgelele să aibă aceeași culoare.
  \item $C_2 = k^{2}$ dacă $n$ este par. Putem alege culorile primelor două mărgele, iar ele vor dicta culorile restului de mărgele.
  \item În general, $C_r = k^{\gcd(r, n)}$.
\end{itemize}
...
Suspectez că unele dintre probleme pot fi rezolvate și cu PIE.

\section{Un puzzle}

Iată și \href{https://gurmeet.net/puzzles/100-prisoners-and-100-boxes/index.html}{un puzzle} care se rezolvă cu una dintre tehnicile de mai sus. Nu spunem care. \emoji{slightly-smiling-face}
